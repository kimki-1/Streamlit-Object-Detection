...
def run_segmentation() :
    st.title('Sementic Segmentation')
    side_radio = st.sidebar.radio('Segmentation Select',['Image Segmentation', 'Video Segmentation'])

    if side_radio == 'Image Segmentation' :
        st.markdown('## <Image Segmentation>')
        st.write('example image test') 
        st.image('data/images/seg_img.PNG')
        st.image('data/images/seg_img2.PNG')
        st.image('data/images/seg_img3.PNG')
        st.image('data/images/seg_img4.PNG')
    ...
    if side_radio == 'Video Segmentation' :
        st.markdown('## <Video Segmentation>')
        st.markdown('#### ※ AWS EC2 프리티어를 사용하기 때문에 실시간 영상플레이가 부드럽지않아서 로컬에서 실행시킨 영상')
        st.write('')
        st.video('data/videos/seg_video2.mp4')
        st.markdown('***')
        st.markdown('## <Result Video>')
        st.write('[원본영상]')
        st.video('data/videos/seg_ori.mp4')
        st.write('[Segmentation 영상]')
        st.video('data/videos/seg_video.mp4')
